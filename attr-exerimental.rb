...
ARGV.each do |f|
   ...
   extent_m = count[1].size
   specificity_m = g.mean_average_shortest_path
   puts "Extent measurements: #{ extent_m }"
   puts "Specificity measurements: #{ specificity_m }"
   puts "Combination (just multiplying both): #{ extent_m * specificity_m }"
end
